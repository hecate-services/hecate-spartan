...
| [019](019_experiment_v1_cross_engine_verification_economics_pre_registration.md) | Experiment V1: cross-engine verification economics (pre-reg, DRAFT, pre-gate) | Self-verification is signed dead (018), so does a *different engine* in the verify seat avoid the failure? Two mechanisms explain 018 and only one makes a second engine useful: self-blindness (a stranger helps) or prompt over-compliance (a "remove every field you cannot confirm" instruction makes any small model over-delete, stranger included). Registers a cheap non-signable pre-flight to fork them **before** spending a corpus, applying 018's own meta-result to its successor. Records five design defects inherited from a stale draft: the context reduction the design rested on does not exist in the instrument, and **a different model is not a different mind** — swapping the engine is intra-agent, so the "inter-mind verification line" the draft claimed to cross is still uncrossed. |
