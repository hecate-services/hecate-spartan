...
| [015](015_experiment_p6_retriever_value_pre_registration.md) | Experiment P6: does the retriever earn its keep? (pre-reg, DRAFT) | 012's surviving positive is external queried memory, and our retriever has never produced a number. Both outcomes delete code: no semantic win over lexical drops the embedder, no chain-gain drops `evolve/2`. Constant-free kills, a degenerate `recency` arm as the gate, and one openly unsolved problem: the probe corpus decides the winner and I would be writing it. |
